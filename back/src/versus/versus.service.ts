...
export class VersusService {
	...
	async createFirstRound(tournament: any) {
		...
		await this.prisma.$transaction(async (prisma) => {
			if (!positionId) {
				const position = await prisma.positionsBattle.create({
					data: {
						round: firstRound,
	...
	async createNextRound(tournament: any) {
		...
		const teamsInLastRound = this.getWinningTeamsFromLastRound(
			tournament,
			lastRound,
		);
		...
		await this.prisma.$transaction(async (prisma) => {
			if (lastRound === Round.SEMIFINAL) {
				await this.createThirdPlaceBattle(
					prisma,
					tournament,
					teamsInLastRound,
				);
				await this.createFinalRound(
					prisma,
					tournament,
					teamsInLastRound,
				);
				...
				await this.createSubsequentRound(
					prisma,
					tournament,
					teamsInLastRound,
					nextRound,
	...
	async createFinalRound(prisma: any, tournament: any, teams: string[]) {
		await prisma.versus.create({
			data: {
				tournament: { connect: { id: tournament.id } },
	...
	getLastRound(tournament: any): Round {
		return tournament.versus.reduce(
			(max, versus) => (max > versus.round ? max : versus.round),
			Round.FIRST_ROUND,
		);
	}
	...
	async updateWinnerRound(winnerId: string, versusId: string) {
		const versusData = await this.prisma.versus.findUnique({
			where: { id: versusId },
		});
