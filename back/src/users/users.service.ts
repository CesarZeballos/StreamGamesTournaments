...
export class UsersService {
	constructor(private readonly prisma: PrismaService) { }
	 ...
	 async getAllUsers(): Promise<User[]> {
	   try {
	     const users = await this.prisma.user.findMany({include: {teamsRequest: true,
			organizedTournaments: true,	
			participatedTournaments:true,
		friends: true,
	friendsRequest: true
}});
	     if (users.length === 0) {
	...
	async getUserByEmail(email: string): Promise<User | null> {
		try {
			...
			throw new InternalServerErrorException(`Failed to get user with email ${email}`)
		}
	}
