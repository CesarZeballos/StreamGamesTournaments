...
export class TeamsService {
	...
	async createTeam(createTeamDto: CreateTeamDto): Promise<Team> {
		...
		return this.prisma.$transaction(async (prisma) => {
			...
			const organizer = await this.fetchs.FindUserByUnique({
				id: organizerId,
			});
			if (!organizer)
				throw new NotFoundException(
					`Organizer with id: ${organizerId} does not exist.`,
				);

			const existingUserIds = tournament.teams.flatMap((team) =>
				team.users.map((user) => user.id),
			);
			const duplicateUsers = userIds.filter((userId) =>
				existingUserIds.includes(userId),
			);
			if (duplicateUsers.length > 0)
				throw new ConflictException(
					`Users with IDs ${duplicateUsers.join(', ')} already exist in the tournament.`,
				);
			...
			if (userIds.length > 0) {
				for (const userId of userIds) {
					const foundUser = await this.fetchs.FindUserByUnique({
						id: userId,
					});
					if (!foundUser)
		...
		return this.prisma.$transaction(async (prisma) => {
			...
			if (userIds) {
				for (const newUserId of userIds) {
					const userExists = await this.fetchs.FindUserByUnique({
						id: newUserId,
					});
					if (!userExists)
						throw new NotFoundException(
					...
					const teamMembers = await this.fetchs.FindTeamByUnique({
						id,
					});
					...
					const userInTeam = await prisma.team.findUnique({
						where: { id },
						include: { users: true },
					});
					if (
						userInTeam.users.some((user) => user.id === newUserId)
					) {
						throw new ConflictException(
	...
	async deleteMember(teamId: string, memberId: string): Promise<Team> {
		...
		if (
			!userInTeam ||
			!userInTeam.users.some((user) => user.id === memberId)
		) {
